...
class Main{
	...
	static function main(){
		...
		float DE(vec3 z) {
			z = vRotateZ(z, time*0.02*M_PI);

			float r;
			int n1 = 0;
			for (int n = 0; n < FRACT_ITER; n++) {
				float rotate = M_PI*time*0.01;
				z = vRotateX(z, rotate);
				//z = vRotateY(z, rotate);
				//z = vRotateZ(z, rotate);
		...
		vec3 getShading(in vec2 distance, in vec3 origin, in vec3 direction, in int object) {
			...
			if (object == 1) {
				color = vec3(1.0, 0.6, cheapAO);
			}
			
			return mix(color, vec3(0.0, 0.1, 0.2), cheapAO);
		}
		...
		void main() {
			...
			vec3 origin = vec3(0.0, 2.0, 0.0) - eyeTranslate/128.0;
			...
			gl_FragColor = vec4(pow(color,1./vec3(2.2)), 1.0);
		}
		';
