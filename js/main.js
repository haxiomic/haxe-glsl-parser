...
glsl_lex_Tokenizer.tokenize = function(source,onWarn,onError) {
	...
	while(glsl_lex_Tokenizer.i < source.length || glsl_lex_Tokenizer.mode != glsl_lex__$Tokenizer_ScanMode.UNDETERMINED) {
		...
		switch(glsl_lex_Tokenizer.mode[1]) {
		case 0:
...
glsl_lex_Tokenizer.literalMode = function() {
	if(glsl_lex_Tokenizer.endConditionsMap.get(glsl_lex_Tokenizer.mode)()) {
		...
		var key = glsl_lex_Tokenizer.buf;
		tt = __map_reserved[key] != null?_this.getReserved(key):_this.h[key];
...
glsl_lex_Tokenizer.floatingConstantMode = function() {
	switch(glsl_lex_Tokenizer.floatMode) {
	case 0:
...
glsl_parse_Actions.s = function(n) {
	...
	return glsl_parse_Parser.stack[glsl_parse_Parser.i - (glsl_parse__$Parser_RuleInfoEntry_$Impl_$.get_nrhs(glsl_parse_Parser.ruleInfo[glsl_parse_Actions.ruleno]) - n)].minor;
};
...
glsl_parse_ParseContext.prototype = {
	...
	,declareType: function(specifier) {
		this.scopes[this.scopes.length - 1].set(specifier.name,glsl_parse_Object.USER_TYPE(specifier));
	}
	,declareVariable: function(declarator) {
		this.scopes[this.scopes.length - 1].set(declarator.name,glsl_parse_Object.VARIABLE(declarator));
	}
...
glsl_preprocess_Preprocessor.prototype = {
	_process: function(inputTokens,forceResolve) {
		...
		while(this.i < this.tokens.length) {
			...
			var _hx_tmp = glsl_lex_TokenHelper.identifierTokenTypes.indexOf(_g) >= 0;
			if(_g[1] == 94) {
				try {
					...
					if( js_Boot.__instanceof($e0,glsl_preprocess_PPError) ) {
						...
						case 0:
							var info = e[3];
							_gthis.note(e[2],info != null?info:_gthis.tokens[_gthis.i]);
							break;
						case 1:
							var info1 = e[3];
							_gthis.warn(e[2],info1 != null?info1:_gthis.tokens[_gthis.i]);
							break;
						case 2:
							var info2 = e[3];
							_gthis.error(e[2],info2 != null?info2:_gthis.tokens[_gthis.i]);
							break;
						}
					} else if( js_Boot.__instanceof($e0,String) ) {
						var msg = $e0;
						_gthis.warn(msg,_gthis.tokens[_gthis.i]);
					} else throw($e0);
				}
			} else if(_hx_tmp == true) {
				try {
					...
					if( js_Boot.__instanceof($e1,glsl_preprocess_PPError) ) {
						...
						case 0:
							var info3 = e1[3];
							_gthis.note(e1[2],info3 != null?info3:_gthis.tokens[_gthis.i]);
							break;
						case 1:
							var info4 = e1[3];
							_gthis.warn(e1[2],info4 != null?info4:_gthis.tokens[_gthis.i]);
							break;
						case 2:
							var info5 = e1[3];
							_gthis.error(e1[2],info5 != null?info5:_gthis.tokens[_gthis.i]);
							break;
						}
					} else if( js_Boot.__instanceof($e1,String) ) {
						var msg1 = $e1;
						_gthis.warn(msg1,_gthis.tokens[_gthis.i]);
					} else throw($e1);
				}
			}
			this.i++;
	...
	,processDirective: function() {
		...
		case "undef":
			...
			this.undefineMacro(glsl_preprocess_Preprocessor.macroNameReg.matched(1));
			if(!this.preserveMacroDefinitions) {
				glsl_lex_TokenHelper.deleteTokens(this.tokens,this.i);
			}
			break;
	...
	,processIfSwitch: function() {
		...
		try {
			...
			try {
				...
				case "ifdef":
					level = 1;
					if(!glsl_preprocess_Preprocessor.macroNameReg.match(_g.content)) {
						throw new js__$Boot_HaxeError("invalid macro name");
					}
			...
			while(_g1 < branches.length) {
				...
				try {
					...
					var tmp = this.userDefinedMacros.keys();
					while(tmp.hasNext()) {
						var k = tmp.next();
						var _this = this.userDefinedMacros;
						var value = __map_reserved[k] != null?_this.getReserved(k):_this.h[k];
						if(__map_reserved[k] != null) {
							userMacrosBefore[0].setReserved(k,value);
						} else {
							userMacrosBefore[0].h[k] = value;
						}
					}
					...
					while(_g2 < _g11) {
						var c = branches[branches.length - 1 - _g2++];
						var _g3 = this.readDirectiveData(c.directiveToken.data);
						if(_g3 != null) {
							switch(_g3.title) {
							case "elif":case "if":
								var directiveTokens = glsl_lex_Tokenizer.tokenize(_g3.content);
								var _g31 = 0;
						...
						var tmp1 = this.userDefinedMacros.keys();
						while(tmp1.hasNext()) {
							var k1 = tmp1.next();
							var _this1 = this.userDefinedMacros;
							var value1 = __map_reserved[k1] != null?_this1.getReserved(k1):_this1.h[k1];
							if(__map_reserved[k1] != null) {
								childUserMacros.setReserved(k1,value1);
							} else {
								childUserMacros.h[k1] = value1;
							}
						}
					...
					var prependTokens = [];
					var tmp2 = requiredMacros[0].keys();
					while(tmp2.hasNext()) {
						var id2 = tmp2.next();
						...
						case 0:
							defineStr = "#define " + id2 + " " + requiredMacro[2];
							break;
						case 1:
							defineStr = "#define " + id2 + "(" + requiredMacro[3].join(", ") + ") " + requiredMacro[2];
							break;
						default:
							continue;
						}
						prependTokens = prependTokens.concat(glsl_lex_Tokenizer.tokenize(undefineStr + "\n" + defineStr + "\n"));
					}
			...
			while(level > 0) {
				...
				switch(this.readDirectiveData(t.data).title) {
				case "endif":
					--level;
					break;
				case "if":case "ifdef":case "ifndef":
	...
	,getMacro: function(id) {
		...
		ppMacro = __map_reserved[id] != null?_this.getReserved(id):_this.h[id];
		if(ppMacro != null) {
			return ppMacro;
		}
		var _this1 = this.userDefinedMacros;
		ppMacro = __map_reserved[id] != null?_this1.getReserved(id):_this1.h[id];
		if(ppMacro != null) {
			return ppMacro;
		}
		return null;
	...
	,defineMacro: function(id,ppMacro) {
		...
		var _hx_tmp = this.isBuiltinMacro(existingMacro);
		var _hx_tmp1 = this.isUserMacro(existingMacro);
		if(_hx_tmp == true) {
			throw new js__$Boot_HaxeError("cannot redefine predefined macro '" + id + "'");
		} else if(_hx_tmp1 == true) {
	...
	,evaluateMacroDefinition: function(definitionString) {
		if(glsl_preprocess_Preprocessor.macroNameReg.match(definitionString)) {
			...
			if(nextChar == "(") {
				...
				if(parametersReg.match(glsl_preprocess_Preprocessor.macroNameReg.matchedRight())) {
	...
	,expandIdentifier: function(tokens,i,overrideMap,ignore) {
		...
		resolveMacro = function(ppMacro1) {
			switch(ppMacro1[1]) {
			case 0:
				var newTokens = glsl_lex_Tokenizer.tokenize(ppMacro1[2],function(warning) {
			...
			case 1:
				...
				try {
					...
					var newTokens1 = glsl_lex_Tokenizer.tokenize(ppMacro1[2],function(warning1) {
				...
				break;
			case 2:
				var newTokens2 = glsl_lex_Tokenizer.tokenize(ppMacro1[2](),function(warning2) {
					throw new js__$Boot_HaxeError("" + warning2);
				},function(error2) {
					throw new js__$Boot_HaxeError("" + error2);
				});
				var _g3 = 0;
				while(_g3 < newTokens2.length) {
					var t2 = newTokens2[_g3];
					++_g3;
					t2.line = token.line;
			...
			case 3:
				...
				try {
					...
					if(functionCall1.args.length != requiredParameterCount) {
						switch(functionCall1.args.length > requiredParameterCount) {
						case false:
							throw new js__$Boot_HaxeError("not enough arguments for macro");
							break;
						case true:
					...
					var newTokens3 = glsl_lex_Tokenizer.tokenize(ppMacro1[2](functionCall1.args),function(warning3) {
						throw new js__$Boot_HaxeError("" + warning3);
					},function(error3) {
						throw new js__$Boot_HaxeError("" + error3);
					});
					var _g4 = 0;
					while(_g4 < newTokens3.length) {
						var t3 = newTokens3[_g4];
						++_g4;
						t3.line = token.line;
	...
	,replaceErrorInfo: function(error,newInfo) {
		...
		if(_g == null) {
			...
			if(_g[2] == glsl_preprocess_PPError) {
				switch(error[1]) {
				case 0:
					return glsl_preprocess_PPError.Note(error[2],newInfo);
				case 1:
					return glsl_preprocess_PPError.Warn(error[2],newInfo);
				case 2:
					return glsl_preprocess_PPError.Error(error[2],newInfo);
				}
			} else {
				return glsl_preprocess_PPError.Warn(error,newInfo);
			}
...
glsl_print_NodePrinter.print = function(n,indentWith,indentLevel) {
	...
	if(_g == null) {
		...
		switch(_g[1]) {
		case 0:
			return glsl_print_RootPrinter.print(_g[2],indentWith,indentLevel);
		case 1:
			return glsl_print_TypeSpecifierPrinter.print(_g[2],indentWith,indentLevel);
		case 2:
			return glsl_print_StructSpecifierPrinter.print(_g[2],indentWith,indentLevel);
		case 3:
			return glsl_print_StructFieldDeclarationPrinter.print(_g[2],indentWith,indentLevel);
		case 4:
			return glsl_print_StructDeclaratorPrinter.print(_g[2],indentWith,indentLevel);
		case 5:
			return glsl_print_ExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 6:
			return glsl_print_IdentifierPrinter.print(_g[2],indentWith,indentLevel);
		case 7:
			return glsl_print_PrimitivePrinter.print(_g[2],indentWith,indentLevel);
		case 8:
			return glsl_print_BinaryExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 9:
			return glsl_print_UnaryExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 10:
			return glsl_print_SequenceExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 11:
			return glsl_print_ConditionalExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 12:
			return glsl_print_AssignmentExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 13:
			return glsl_print_FieldSelectionExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 14:
			return glsl_print_ArrayElementSelectionExpressionPrinter.print(_g[2],indentWith,indentLevel);
		case 15:
			return glsl_print_FunctionCallPrinter.print(_g[2],indentWith,indentLevel);
		case 16:
			return glsl_print_ConstructorPrinter.print(_g[2],indentWith,indentLevel);
		case 17:
			return glsl_print_DeclarationPrinter.print(_g[2],indentWith,indentLevel);
		case 18:
			return glsl_print_PrecisionDeclarationPrinter.print(_g[2],indentWith,indentLevel);
		case 19:
			return glsl_print_VariableDeclarationPrinter.print(_g[2],indentWith,indentLevel);
		case 20:
			return glsl_print_DeclaratorPrinter.print(_g[2],indentWith,indentLevel);
		case 21:
			return glsl_print_ParameterDeclarationPrinter.print(_g[2],indentWith,indentLevel);
		case 22:
			return glsl_print_FunctionDefinitionPrinter.print(_g[2],indentWith,indentLevel);
		case 23:
			return glsl_print_FunctionPrototypePrinter.print(_g[2],indentWith,indentLevel);
		case 24:
			return glsl_print_FunctionHeaderPrinter.print(_g[2],indentWith,indentLevel);
		case 25:
			return glsl_print_StatementPrinter.print(_g[2],indentWith,indentLevel);
		case 26:
			return glsl_print_CompoundStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 27:
			return glsl_print_DeclarationStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 28:
			return glsl_print_ExpressionStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 29:
			return glsl_print_IterationStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 30:
			return glsl_print_WhileStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 31:
			return glsl_print_DoWhileStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 32:
			return glsl_print_ForStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 33:
			return glsl_print_IfStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 34:
			return glsl_print_JumpStatementPrinter.print(_g[2],indentWith,indentLevel);
		case 35:
			return glsl_print_ReturnStatementPrinter.print(_g[2],indentWith,indentLevel);
		default:
			throw new js__$Boot_HaxeError("Node cannot be printed: " + Std.string(n));
		}
	}
			...
			if(nextNodeEnum != null) {
				...
				if(currentNodeEnum[1] == 19) {
					if(nextNodeEnum[1] == 19) {
						...
						if(!Type.enumEq(c.typeSpecifier != null?c.typeSpecifier.storage:null,n1.typeSpecifier != null?n1.typeSpecifier.storage:null)) {
							unit += "\n";
						}
					}
				}
...
glsl_print_StructSpecifierPrinter.print = function(n,indentWith,indentLevel) {
	...
	str += "struct " + (n.name != null?n.name:"") + "{" + (pretty?"\n":"");
...
glsl_print_StructDeclaratorPrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent(n.name + (n.arraySizeExpression != null?"[" + glsl_print_ExpressionPrinter.print(n.arraySizeExpression,indentWith,0) + "]":""),indentWith,indentLevel);
};
...
glsl_print_DeclarationPrinter.print = function(n,indentWith,indentLevel) {
	...
	if(_g == null) {
		...
		case 23:
			return glsl_print_FunctionPrototypePrinter.print(_g[2],indentWith,indentLevel);
		case 36:
			return glsl_print_PreprocessorDirectivePrinter.print(_g[2],indentWith,indentLevel);
		default:
			throw new js__$Boot_HaxeError("Declaration cannot be printed: " + Std.string(n));
		}
	}
...
glsl_print_PrecisionDeclarationPrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent("precision " + glsl_print_PrecisionQualifierPrinter.print(n.precision) + " " + glsl_print_DataTypePrinter.print(n.dataType) + ";",indentWith,indentLevel);
};
...
glsl_print_ParameterDeclarationPrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent(parts.join(" "),indentWith,indentLevel);
};
...
glsl_print_FunctionPrototypePrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent(glsl_print_FunctionHeaderPrinter.print(n.header,indentWith) + ";",indentWith,indentLevel);
};
...
glsl_print_DeclarationStatementPrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent(glsl_print_DeclarationPrinter.print(n.declaration,indentWith),indentWith,indentLevel);
};
...
glsl_print_IfStatementPrinter.print = function(n,indentWith,indentLevel) {
	...
	var pretty = indentWith != null;
	var compoundConsequent = glsl_NodeTypeHelper.safeNodeType(n.consequent)[1] == 26;
	...
	if(n.alternate != null) {
		str += pretty && !compoundConsequent?"\n":"";
		var compoundAlternate = glsl_NodeTypeHelper.safeNodeType(n.alternate)[1] == 26;
		str += "else";
...
glsl_print_DoWhileStatementPrinter.print = function(n,indentWith,indentLevel) {
	...
	var pretty = indentWith != null;
	var compoundBody = glsl_NodeTypeHelper.safeNodeType(n.body)[1] == 26;
	var str = "do";
	str = "do" + (!compoundBody?" ":"");
	str += glsl_print_StatementPrinter.print(n.body,indentWith);
...
glsl_print_ForStatementPrinter.print = function(n,indentWith,indentLevel) {
	...
	var pretty = indentWith != null;
	var str = "for";
	str = "for" + ("(" + (n.init != null?glsl_print_StatementPrinter.print(n.init,indentWith):"") + (pretty?" ":"") + (n.test != null?glsl_print_ExpressionPrinter.print(n.test,indentWith):"") + (pretty?"; ":";") + (n.update != null?glsl_print_ExpressionPrinter.print(n.update,indentWith):"") + ")");
	str += glsl_print_StatementPrinter.print(n.body,indentWith);
	return glsl_print_Utils.indent(str,indentWith,indentLevel);
};
...
glsl_print_PreprocessorDirectivePrinter.print = function(n,indentWith,indentLevel) {
	...
	return glsl_print_Utils.indent(n.content,indentWith,indentLevel);
};
...
glsl_print_Utils.indent = function(str,chars,level) {
	...
	while(_g21 < _g1) {
		var i = _g21++;
		result += identStr + lines[i] + (i < lines.length - 1?"\n":"");
	}
	return result;
};
